...
public class oscillator extends Block implements ITileEntityProvider{
        ...
        if (worldIn.getBlockState(pos).getValue(TYPE).equals(0)){
            if(worldIn.isBlockPowered(pos)){
                ...
                if(blockNorth.equals(synthBlocks.speaker)){
                    worldIn.playSound((EntityPlayer)null, northPos, SoundEvents.block_note_harp, SoundCategory.BLOCKS, 3.0F, pitch);
                }
                else if(blockSouth.equals(synthBlocks.speaker)){
                    worldIn.playSound((EntityPlayer)null, southPos, SoundEvents.block_note_harp, SoundCategory.BLOCKS, 3.0F, pitch);
                }
                else if(blockWest.equals(synthBlocks.speaker)){
                    worldIn.playSound((EntityPlayer)null, westPos, SoundEvents.block_note_harp, SoundCategory.BLOCKS, 3.0F, pitch);
                }
                    ...
                    worldIn.playSound((EntityPlayer)null, downPos, SoundEvents.block_note_harp, SoundCategory.BLOCKS, 3.0F, pitch);
                }
            }
        }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (heldItem != null) {
            if (heldItem.areItemsEqual(new ItemStack(synthItems.screwdriver), heldItem)) {
            ...
            } else if (heldItem.areItemsEqual(new ItemStack(synthItems.tuner), heldItem)) {
                if(pitch >= 2.0F){
                    pitch = 0.0F;
                }else{
                    pitch = pitch + 0.1F;
    ...
    public static void setPitchFromNBT(float pitchF){
        pitch = pitchF;
    }
